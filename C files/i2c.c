...
void i2c_init(){
	...
	uint8_t Prescaler = pow(2,(2*(TWSR & 0b11)));//solo para tomar en cuenta los dos bits menos significativos ya que el resto puede cambiar
	TWBR = ((F_CPU / F_SCL)-16)/(2*Prescaler);//formula de la frecuencia de reloj del i2c que debe ser 100KHz segun el fabricante de los slave
